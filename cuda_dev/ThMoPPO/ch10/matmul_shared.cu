...
__global__ void MatrixMulGPU_Preload(const float* Ad, const float* Bd, float* Cd, int width) {
    uint row = blockIdx.y * blockDim.y + threadIdx.y;
    uint col = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    __shared__ float ldsa[2048];
    __shared__ float ldsb[2048];

    float gl_a = Ad[ldsm_row * width + ldsm_col];
    float gl_b = Bd[ldsn_row * width + ldsn_col];

    ldsa[offset_inner] = gl_a;
    ldsb[offset_inner] = gl_b;
    __syncthreads();

    for (int i{ 8 }; i < width; i += 8) {
        gl_a = Ad[ldsm_row * width + ldsm_col + i];
        gl_b = Bd[(ldsn_row + i) * width + ldsn_col];
        
        uint stride_j = (((i / 8) + 1) % 2) * 1024;
        uint stride_i = ((i / 8) % 2) * 1024;

        for (int j{ 0 }; j < 8; ++j) {
            rA[0] = ldsa[threadIdx.y * 4 + (j * 128) + 0 + stride_j];
            rA[1] = ldsa[threadIdx.y * 4 + (j * 128) + 1 + stride_j];
            rA[2] = ldsa[threadIdx.y * 4 + (j * 128) + 2 + stride_j];
            rA[3] = ldsa[threadIdx.y * 4 + (j * 128) + 3 + stride_j];

            rB[0] = ldsb[threadIdx.x * 4 + (j * 128) + 0 + stride_j];
            rB[1] = ldsb[threadIdx.x * 4 + (j * 128) + 1 + stride_j];
            rB[2] = ldsb[threadIdx.x * 4 + (j * 128) + 2 + stride_j];
            rB[3] = ldsb[threadIdx.x * 4 + (j * 128) + 3 + stride_j];
        ...
        ldsa[offset_inner + stride_i] = gl_a;
        ldsb[offset_inner + stride_i] = gl_b;
        __syncthreads();
    }

    for (int j{ 0 }; j < 8; ++j) {
        rA[0] = ldsa[threadIdx.y * 4 + (j * 128) + 0 + 1024];
        rA[1] = ldsa[threadIdx.y * 4 + (j * 128) + 1 + 1024];
        rA[2] = ldsa[threadIdx.y * 4 + (j * 128) + 2 + 1024];
        rA[3] = ldsa[threadIdx.y * 4 + (j * 128) + 3 + 1024];

        rB[0] = ldsb[threadIdx.x * 4 + (j * 128) + 0 + 1024];
        rB[1] = ldsb[threadIdx.x * 4 + (j * 128) + 1 + 1024];
        rB[2] = ldsb[threadIdx.x * 4 + (j * 128) + 2 + 1024];
        rB[3] = ldsb[threadIdx.x * 4 + (j * 128) + 3 + 1024];
...
int main() {
    ...
    dim3 bDim{32, 32};
    dim3 gDim{32, 32};
    MatrixMulGPU_Shared <<< gDim, bDim >>> (Ag, Bg, Cg, Width);
    ...
    dim3 bDim2{32, 32};
    dim3 gDim2{32, 32};
    MatrixMulGPU_Preload <<< gDim2, bDim2 >>> (Ag, Bg, Cg, Width);
    CHECK(cudaDeviceSynchronize());
    elaps = seconds() - start;
    printf("MatrixMulGPU_Preload <<< (%d, %d), (%d, %d) >>> elaps %f sec\n", gDim2.x, gDim2.y, bDim2.x, bDim2.y, elaps);
